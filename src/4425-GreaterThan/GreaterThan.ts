...
type SameLength<T extends number , U extends number> = `${T}` extends `${infer TF extends number}${infer TR}` ? `${U}` extends `${infer UF extends number}${infer UR}` ? false : true :true

type GreaterThan<T extends number, U extends number> = LengthOfString<`${T}`> extends LengthOfString<`${U}`> ? SameLength<T,U> : 
